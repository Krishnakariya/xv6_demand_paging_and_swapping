  ...
  for(b = 0; b < sb.size; b += BPB){
   begin_op();
    ...
    for(bi = 0; bi < BPB && b + bi < sb.size; bi+=8){
        ...
        // Race cond: trying to take some lock and increment numallocblocks.
 ...
 */
void
bfree_page(int dev, uint b)// b should be multiple of 8.
{
 struct buf *bp;
  int bi, m;
  ...
  brelse(bp);
   ...
   // race condition: trying to take some lock and decrement numallocblocks. 
